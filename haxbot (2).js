	...
	const roomName = "⚽ 𝑬𝒍𝒆𝒗𝒆𝒏 𝑨𝒓𝒆𝒏𝒂 | 𝒙3";
	const botName = "Juiz";
	const maxPlayers = 25;
	const roomPublic = true;
	const geo = [{"lat":-26.5716,"lon":-53.5943,"code":"br"}];
	...
	function endGame(winner) { // handles the end of a game : no stopGame function inside
		...
		if (winner == Team.RED) {
			streak++;
			room.sendAnnouncement("🏆 Time vermelho venceu! [" + scores.red + " - " + scores.blue + "] | Sequência de Vitória(s): " + streak + " 🏆", null, 0xFDC43A);
		}
		else if (winner == Team.BLUE) {
			streak = 1;
			room.sendAnnouncement("🏆 Time azul venceu! [" + scores.blue + " - " + scores.red + "] | Sequência de Vitória(s): " + streak + " 🏆", null, 0xFDC43A);
		}
		else {
			streak = 0;
	...
	function handleInactivity() { // handles inactivity : players will be kicked after afkLimit
		...
		for (var i = 0; i < extendedP.length ; i++) {
			if (extendedP[i][eP.ACT] == 60 * (2/3 * afkLimit)) {
				room.sendAnnouncement("[PV] ⛔ @" + room.getPlayer(extendedP[i][eP.ID]).name + ", se você não se mexer nos próximos " + Math.floor(afkLimit / 3) + " segundos, você sera kickado!", extendedP[i][eP.ID]);
			}
	...
	room.onPlayerJoin = function(player) {
		...
		sendAnnouncementToDiscord(
			"```"+"[📝] Informações do jogador, conn, auth e data ⏰" + "\n"+
		...
		if (localStorage.getItem(player.auth) != null) {
			if (JSON.parse(localStorage.getItem(player.auth))[Ss.RL] != "player") {
				...
				room.sendAnnouncement((JSON.parse(localStorage.getItem(player.auth))[Ss.RL] == "master" ? "O Administrador " : "O Administrador ") + player.name + " se conectou!", null, 0xD81826);
			}
		}
	}
	...
	room.onPlayerChat = function (player, message) {
		if (message.length > 1 && message[0].toLowerCase() == 't' && message[1] == ' ') {
			...
			room.sendAnnouncement("Você não está em um time.")
			}}
		...
		if(message == '!limparchat'){
			...
			room.sendAnnouncement("[🏆] Seus stats: 🎮 Jogos: " + stats[Ss.GA] + ", ✅ Vitórias: " + stats[Ss.WI] + ", ⭕ Empates: " + stats[Ss.DR] + ", ❌ Derrotas: " + stats[Ss.LS] + ", Porcentagem de vitórias: " + stats[Ss.WR] + "%, ⚽️ Gols: " + stats[Ss.GL] + ", 👟 Assistências: " +stats[Ss.AS] + ", 🤚 GK: " + stats[Ss.GK] + ", 🤚 CS: " + stats[Ss.CS] + ", 🤚 CS%: " + stats[Ss.CP] + "%", player.id,0x73EC59,"bold"); 
			}

			else if (["!mostrarme"].includes(message[0].toLowerCase())) {
				var stats;
				localStorage.getItem(getAuth(player)) ? stats = JSON.parse(localStorage.getItem(getAuth(player))) : stats = [0, 0, 0, 0, "0.00", 0, 0, 0, 0, "0.00"]; 
				room.sendAnnouncement("[🏆] Stats do " + player.name + ": 🎮 Jogos: " + stats[Ss.GA] + ", ✅ Vitórias: " + stats[Ss.WI] + ", ⭕ Empates: " + stats[Ss.DR] + ", ❌ Derrotas: " + stats[Ss.LS] + ", Porcentagem de vitórias: " + stats[Ss.WR] + "%, ⚽️ Gols: " + stats[Ss.GL] + ", 👟 Assistências: " +stats[Ss.AS] + ", 🤚 GK: " + stats[Ss.GK] + ", 🤚 CS: " + stats[Ss.CS] + ", 🤚 CS%: " + stats[Ss.CP] + "%", null,0x73EC59,"bold"); 
				}
		...
		tableau.sort(function (a, b) { return b[1] - a[1]; });
		room.sendAnnouncement("[🏆] ✅ Vitorias> #1 " + tableau[0][0] + ": " + tableau[0][1] + " #2 " + tableau[1][0] + ": " + tableau
		...
		return false;
		...
		else if (["!passes"].includes(message[0].toLowerCase())) { // mostra o ranking de assistencias
			var tableau = [];
			try{
			Object.keys(localStorage).forEach(function (key) { if (!["player_name", "view_mode", "geo", "avatar", "player_auth_key"].
			...
			tableau.sort(function (a, b) { return b[1] - a[1]; });
			room.sendAnnouncement("[🏆] 👟 Assistências> #1 " + tableau[0][0] + ": " + tableau[0][1] + " #2 " + tableau[1][0] + ": " +
			tableau[1][1] + " #3 " + tableau[2][0] + ": " + tableau[2][1] + " #4 " + tableau[3][0] + ": " + tableau[3][1] + " #5 " +
			tableau[4][0] + ": " + tableau[4][1], player.id,0x73EC59);
			return false;
			}
	...
	return false;
			...
			room.sendAnnouncement("                                        💬 Discord Link: ➡ https://discord.gg/EmRsPPh4/ ⬅", null, 0xF6FF43, 'bold');
		}
	...
	room.onGameStart = function(byPlayer) {
		...
		allReds = [];
		allBlues = [];
		room.sendAnnouncement("Uniformes: !atl, !ast, !tot, !ars, !mun, !mci, !ita, !che, !juv, !psg, !gre, !ibi, !bor, !liv, !fra, !rea, !vas, !cori, !cru, !hol, !san, !spfc, !fla, !bra, !bay, !mil, !bot, !esp.", null, Cor.Verde, Normal);
		room.sendAnnouncement("[💬] Use t para conversar com a sua equipe!", null, 0x5EE7FF);
